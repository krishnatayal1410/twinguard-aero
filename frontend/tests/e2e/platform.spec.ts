...
test.beforeEach(async({page})=>{await page.goto("/");await expect(page.getByRole("heading",{name:"Command Center"})).toBeVisible()});
...
test("all primary screens are reachable",async({page})=>{
 ...
 for(const[button,heading]of pages){await page.getByRole("button",{name:button,exact:true}).click();await expect(page.getByRole("heading",{name:heading,exact:true})).toBeVisible()}
});

test("command center mode toggle and navigation work",async({page})=>{
 await page.getByRole("button",{name:"System Map"}).click();
 await expect(page.getByText("CRANKCASE",{exact:true})).toBeVisible();
 await page.getByRole("button",{name:"Open Mission Lab"}).click();
