...
const OFFSETS: Record<ModuleName, [number, number, number]> = {
  core: [0, 0, 0],
  cylinders: [0.95, 0.1, 0],
  injection: [0, 0.85, 0.08],
  turbo: [0, -0.15, 1],
  cooling: [-0.75, 0.45, -0.3],
  lubrication: [0, -0.75, -0.1],
  internals: [-0.85, -0.05, 0.15],
};
const LABEL_POSITIONS: Record<ModuleName, [number, number, number]> = {
  core: [0, 0.25, -0.55],
  cylinders: [1.55, 0.75, 0.1],
  injection: [0.2, 1.7, 0.25],
  turbo: [0.5, 0.35, 1.65],
  cooling: [-1.55, 1.15, -0.55],
  lubrication: [0.1, -1.45, -0.2],
  internals: [-1.55, -0.35, 0.5],
};
const LABELS: Record<ModuleName, string> = {
  core: "Crankcase",
  cylinders: "Cylinder banks",
  injection: "Fuel injection",
  turbo: "Turbocharger",
  cooling: "Cooling circuit",
  lubrication: "Lubrication",
  internals: "Internal drive",
};
  ...
  useEffect(() => {
    root.traverse((object) => {
      ...
      const owner = MODULES.find((module) => {
        ...
        while (parent) {
          if (parent.name.toLowerCase() === module) return true;
      ...
      for (const source of materials) {
        ...
        const visibleOpacity =
          mode === "xray"
            ? selected
              ? Math.max(0.64, opacity)
              : Math.min(0.26, opacity)
            : hasSelection
              ? selected
                ? opacity
                : 0.12
              : opacity;
        material.transparent = visibleOpacity < 0.99;
        material.opacity = visibleOpacity;
        material.depthWrite = visibleOpacity > 0.55;
  ...
  return (
    <>
      {MODULES.map((module) => {
        ...
        return (
          <Html key={module} position={position} center distanceFactor={9}>
            <div className="engine-part-label">{LABELS[module]}</div>
          </Html>
        );
      })}
    </>
...
function FaultMarker({ module, active }: { module: ModuleName; active: boolean }) {
  ...
  return (
    <group position={[position[0] * 0.45, position[1] * 0.45, position[2] * 0.45]}>
      <mesh>
        <sphereGeometry args={[0.12, 24, 24]} />
        <meshBasicMaterial color="#ef4444" toneMapped={false} />
      </mesh>
      <Html center distanceFactor={8}>
        <div className="engine-fault-label">
          <i />
          FAULT LOCATION
        </div>
      </Html>
  ...
  return (
    <Html center>
      <div className="engine-loader">
        <i />
        <span>Loading engineering model…</span>
      </div>
    </Html>
  );
}
...
function Scene({
  ...
  return (
    <>
      ...
      <hemisphereLight
        args={[darkStage ? "#cce7ff" : "#ffffff", darkStage ? "#06111b" : "#b9cad6", darkStage ? 1.8 : 2.4]}
      />
      <directionalLight position={[5, 8, 6]} intensity={3.2} castShadow />
      <directionalLight position={[-6, 2, -4]} intensity={darkStage ? 2.2 : 1.6} color="#60a5fa" />
...
export default function EngineTwin({
  ...
  explode = false,
  xray = false,
  focus: externalFocus = "all",
  autoRotate: externalRotate = false,
  resetToken = 0,
  onFocus,
}: Props) {
  ...
  const [opacity, setOpacity] = useState(100),
    ...
    [autoRotate, setAutoRotate] = useState(externalRotate),
    [fullscreen, setFullscreen] = useState(false),
    [reset, setReset] = useState(0);
  const [explodeAmount, setExplodeAmount] = useState(110),
    [wireframe, setWireframe] = useState(false),
    [labels, setLabels] = useState(false),
    [darkStage, setDarkStage] = useState(false);
  ...
  if (typeof document !== "undefined" && !supportsWebGL())
    return (
      <div className="webgl-fallback">
        <strong>3D engine viewer unavailable</strong>
        <span>Enable browser hardware acceleration and reload.</span>
      </div>
    );
  ...
  return (
    <div
      ref={host}
      className={`engine-viewer ${compact ? "compact " : ""}${fullscreen ? "fullscreen " : ""}${darkStage ? "dark-stage" : ""}`}
    >
      <div className="engine-toolbar" role="toolbar" aria-label="3D engine controls">
        <div className="engine-mode-switch">
          ...
          <button className={mode === "xray" ? "active" : ""} onClick={() => setMode("xray")}>
            <Scan /> X-ray
          </button>
        </div>
        <div className="engine-tool-actions">
          <button className="fault-locate" onClick={locateFault} title="Locate predicted fault">
            <Crosshair />
          ...
          <button
            ...
            className={darkStage ? "active" : ""}
            onClick={() => setDarkStage((value) => !value)}
            title="Toggle inspection stage"
          >
            {darkStage ? <Sun /> : <Moon />}
          </button>
          <button
          ...
          <button onClick={() => setReset((value) => value + 1)} title="Reset camera">
            <Focus />
          </button>
          <button onClick={toggleFull} title={fullscreen ? "Exit full screen" : "Full screen"}>
      ...
      <div className="engine-sliders">
        <div className="engine-opacity">
          <ZoomOut />
          <label>
            <span>Casing opacity</span>
            <input
              aria-label="Engine casing opacity"
              type="range"
              min="8"
              max="100"
              value={opacity}
        ...
        {mode === "exploded" && (
          <div className="engine-opacity engine-explode-range">
            <Move3d />
            <label>
      ...
      {!compact && (
        <div className="engine-parts">
          <div className="engine-parts-head">
            <span>COMPONENT INSPECTOR</span>
            {focus !== "all" && <button onClick={() => setSelected("all")}>Show all</button>}
          </div>
          {MODULES.map((module) => (
            <button
              key={module}
              className={focus === module ? "active" : ""}
              onClick={() => setSelected(focus === module ? "all" : module)}
            >
              <i style={{ background: healthFor(module, twin) < 86 ? "#f59e0b" : "#20a47b" }} />
              <span>{LABELS[module]}</span>
              <b>{fmt(healthFor(module, twin), 0)}%</b>
            </button>
          ))}
        </div>
      )}
      {fullscreen && (
        <aside className="engine-diagnostic-drawer">
          <small>LIVE DIAGNOSTIC</small>
          <h3>{twin?.ai.anomaly ? pretty(twin.ai.probable_fault) : "Engine operating normally"}</h3>
          <button className="drawer-locate" onClick={locateFault}>
            <Crosshair /> Reveal predicted fault
          </button>
          <div className="engine-health-ring">
            <b>{fmt(twin?.health.overall, 0)}%</b>
            <span>overall health</span>
          </div>
          <dl>
            <div>
              <dt>RPM</dt>
              <dd>{fmt(twin?.telemetry.rpm, 0)}</dd>
            </div>
            <div>
              <dt>Oil pressure</dt>
              <dd>{fmt(Number(twin?.telemetry.oil_pressure || 0) * 100, 1)} kPa</dd>
            </div>
            <div>
              <dt>CHT</dt>
              <dd>{fmt(twin?.telemetry.cht, 1)} °C</dd>
            </div>
            <div>
              <dt>RUL</dt>
              <dd>{fmt(twin?.ai.rul_hours, 1)} h</dd>
            </div>
          </dl>
          <p>
            {mode === "xray"
              ? "Fault marker stays visible through the transparent casing."
              : "Use Reveal predicted fault to isolate the affected subsystem."}
          </p>
        </aside>
      )}
    </div>
