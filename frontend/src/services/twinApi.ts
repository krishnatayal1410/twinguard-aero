...
export function connectTwin(
 ...
 const open=()=>{
  ...
  if(!token){onStatus(false);return}
  ws=new WebSocket(`${proto}://${host}/api/v1/ws/twin/ENGINE-01?token=${encodeURIComponent(token)}`);
  ws.onopen=()=>{retry=900;onStatus(true)};
  ws.onmessage=e=>{
   try{
    const x=JSON.parse(e.data);
    if(x?.telemetry){onState(x);if(x.runtime_validity)onValidity?.(x.runtime_validity)}
    else if(x?.runtime_validity)onValidity?.(x.runtime_validity);
   }catch{}
  };
  ws.onclose=()=>{onStatus(false);if(!stopped){setTimeout(open,retry);retry=Math.min(6000,retry*1.5)}};
  ws.onerror=()=>ws?.close();
 };
 open();
