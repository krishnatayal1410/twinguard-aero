...
if [ -d frontend/node_modules ];then (cd frontend&&npm audit --omit=dev --audit-level=high);else echo "Skipped: frontend/node_modules absent";fi
echo "[4] pip-audit (when installed)"
if command -v pip-audit >/dev/null;then pip-audit -r backend/requirements.txt;else echo "Skipped: install pip-audit for CVE database scan";fi
