...
try:
    ...
    auth = post(
        "/api/v1/auth/signup",
        {"name": "Automated Verifier", "email": email, "password": "VerifyTwin9!"},
    )
    TOKEN = auth["token"]
...
def maintenance_check():
    ...
    assert (
        result["rul_interval_hours"]["lower"] <= result["rul_hours"] <= result["rul_interval_hours"]["upper"]
    )
...
def lubrication_fault():
    ...
    try:
        while time.time() < deadline:
            time.sleep(1)
            state = get("/api/v1/twin/ENGINE-01")
            event_seen = event_seen or any(
                e.get("type") in {"ANOMALY_DETECTED", "FAULT_IDENTIFIED"} for e in state.get("events", [])
            )
