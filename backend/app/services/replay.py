...
class ReplayService:
    ...
    def samples(self,mid,limit=5000):
        with SessionLocal() as s:
            if not s.get(MissionRun,mid): return None
            rows=list(s.scalars(select(MissionSample).where(MissionSample.mission_id==mid).order_by(MissionSample.timestamp).limit(max(1,min(limit,10000)))))
            return [self.serialize_sample(x) for x in rows]
    def analyze(self,samples):
        if not samples:return {"sample_count":0,"duration_seconds":0,"events":[]}
        ...
        for x in samples:
            ...
            if x.health<prev_health-4:events.append({"timestamp":x.timestamp.isoformat(),"type":"HEALTH_DEGRADATION","severity":"warning","message":f"Health dropped to {x.health:.1f}/100."})
            if x.maintenance!=prev_maint:events.append({"timestamp":x.timestamp.isoformat(),"type":"MAINTENANCE_CHANGE","severity":"critical" if x.maintenance in {"NO_GO","HIGH"} else "warning","message":f"Maintenance priority changed to {x.maintenance}."})
            prev_anom=x.anomaly;prev_fault=x.fault;prev_maint=x.maintenance;prev_health=x.health
        duration=max(0.0,(samples[-1].timestamp-samples[0].timestamp).total_seconds()) if len(samples)>1 else 0.0
        return {"sample_count":len(samples),"duration_seconds":duration,"start_health":samples[0].health,"end_health":samples[-1].health,"rul_change_hours":samples[-1].rul-samples[0].rul,"max_cht":max(x.cht for x in samples),"min_oil_pressure":min(x.oil_pressure for x in samples),"max_vibration":max(x.vibration for x in samples),"anomaly_samples":sum(x.anomaly for x in samples),"faults_observed":sorted({x.fault for x in samples if x.fault!="normal"}),"events":events}
    @staticmethod
    def serialize_sample(x):
        return {"timestamp":x.timestamp.isoformat(),"health":float(x.health),"rul":float(x.rul),"cht":float(x.cht),"oil_pressure":float(x.oil_pressure),"vibration":float(x.vibration),"anomaly":bool(x.anomaly),"fault":x.fault,"maintenance":x.maintenance}
    @staticmethod
    def serialize(r):
