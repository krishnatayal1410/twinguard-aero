...
from ..db import AuthSession, SessionLocal, UserAccount
...
def validate_password(password: str):
    ...
    if not re.search(r"[A-Z]", password):
        raise AuthError("Password needs an uppercase letter.")
    if not re.search(r"[a-z]", password):
        raise AuthError("Password needs a lowercase letter.")
    if not re.search(r"\d", password):
        raise AuthError("Password needs a number.")
...
def signup(name: str, email: str, password: str) -> tuple[str, dict]:
    if not settings.allow_signup:
        raise AuthError("Account creation is disabled in this environment.")
