...
def test_engineering_fallback_does_not_mix_native_models(monkeypatch):
    monkeypatch.setenv("TWINGUARD_NATIVE_ML", "0")
    engine = AIEngine("models")
    assert engine.native_ml is False
    assert engine.anomaly is None
    assert engine.fault is None
    assert engine.rul is None
...
def test_mission_analysis_direction():
    ...
    assert (
        result["current_rul_interval_hours"]["lower"]
        <= result["current_rul_hours"]
        <= result["current_rul_interval_hours"]["upper"]
    )
    assert (
        result["post_mission_rul_interval_hours"]["lower"]
        <= result["post_mission_rul_hours"]
        <= result["post_mission_rul_interval_hours"]["upper"]
    )
